(() => {
  ...
  function savedPerson(module){ return (state[module]||{}).person || {name:state.patient.name||'',age:state.patient.age||''}; }
  function identityMarkup(id,person={},includeAge=true){ return `<article class="card"><h3>ข้อมูลผู้รับการประเมิน</h3><div class="field-grid"><label>ชื่อ-นามสกุล *<input id="${id}Name" type="text" autocomplete="name" value="${safe(person.name)}" required></label>${includeAge?`<label>อายุ (ปี) *<input id="${id}Age" type="number" min="0" max="130" inputmode="numeric" value="${safe(person.age)}" required></label>`:''}</div></article>`; }
  function renderIdentity(id,person,includeAge=true){ $(`#${id}`).innerHTML=identityMarkup(id,person,includeAge); }
  function readIdentity(id,includeAge=true){ const name=$(`#${id}Name`)?.value.trim()||''; const age=includeAge?$(`#${id}Age`)?.value:''; if(!name|| (includeAge && age===''))return null; return {name,age}; }
  ...
  function assessmentPayload(type){
    ...
    if(type==='osteoporosis') return {assessmentType:'ประเมินความเสี่ยงโรคกระดูกพรุนเบื้องต้น (OSTA)',submittedAt,...(state.osteoporosis||{})};
    if(type==='stepTest') return {assessmentType:'2 Minute Step Test',submittedAt,...(state.stepTest||{})};
    if(type==='frailty') return {assessmentType:'คัดกรองภาวะเปราะบาง (T-FRAIL)',submittedAt,...(state.frailty||{})};
  ...
  function showStep(step){
    state.step = Math.max(0, Math.min(14, step)); save();
  ...
  $('#screeningBackHome').addEventListener('click',()=>showStep(0));
  ...
  $('#startHomeSafety').addEventListener('click',()=>{ $('#homeSafetyForm').reset(); renderIdentity('homeSafetyIdentity',savedPerson('homeSafety')); $('#homeSafetyResult').hidden=true; showStep(7); });
  $('#backHome').addEventListener('click',()=>showStep(0));
  $('#homeSafetyForm').addEventListener('submit',e=>{ e.preventDefault(); const person=readIdentity('homeSafetyIdentity'); if(!person){alert('กรุณากรอกชื่อและอายุผู้รับการประเมิน');return;} const count=$$('input[name="homeRisk"]:checked').length; const result=$('#homeSafetyResult'); let level, advice, color; if(count<=2){level='🏠 บ้านควรระวัง'; advice='พบจุดเสี่ยงไม่เกิน 2 ข้อ ควรติดตามและปรับปรุงจุดเสี่ยงที่พบเพื่อป้องกันการหกล้ม'; color='house-green';}else if(count<=4){level='🏠 บ้านควรแก้ไข'; advice='พบจุดเสี่ยง 3–4 ข้อ ควรวางแผนแก้ไขจุดเสี่ยงภายในบ้าน'; color='house-yellow';}else{level='🏠 บ้านอันตราย แก้ไขด่วน'; advice='พบจุดเสี่ยงตั้งแต่ 5 ข้อขึ้นไป ควรเร่งแก้ไขและปรับสภาพแวดล้อมเพื่อความปลอดภัย'; color='house-red';} state.homeSafety={...(state.homeSafety||{}),person};save(); result.className=`home-result ${color}`; result.innerHTML=`<h3>${level}</h3><p>ผู้รับการประเมิน: <b>${safe(person.name)}</b> · อายุ ${safe(person.age)} ปี</p><p>พบจุดเสี่ยงทั้งหมด <b>${count} ข้อ</b></p><p>${advice}</p>`; result.hidden=false; result.scrollIntoView({behavior:'smooth',block:'center'}); });
  const factorItems=[
  ...
  $('#startFactorRisk').addEventListener('click',()=>{ renderFactorQuestions(); renderIdentity('factorIdentity',savedPerson('factorRisk')); $('#factorResult').hidden=true; error('#factorError',''); showStep(8); });
  $('#factorBackHome').addEventListener('click',()=>showStep(0));
  $('#factorRiskForm').addEventListener('submit',e=>{ e.preventDefault(); const person=readIdentity('factorIdentity'); if(!person){error('#factorError','กรุณากรอกชื่อและอายุผู้รับการประเมิน');return;} const scores=factorItems.map((_,index)=>radio(`factor${index}`)); if(scores.some(score=>score==='')){error('#factorError','กรุณาเลือกคำตอบให้ครบทั้ง 11 ข้อ');return;} error('#factorError',''); const total=scores.reduce((sum,score)=>sum+Number(score),0); let title,detail,color; if(total<=10){title='ความเสี่ยงล้มระดับต่ำ';detail='คะแนนรวม 0–10 คะแนน';color='house-green';}else if(total<=20){title='ความเสี่ยงล้มระดับปานกลาง';detail='คะแนนรวม 11–20 คะแนน';color='house-yellow';}else{title='ความเสี่ยงล้มระดับสูง';detail='คะแนนรวม 21–33 คะแนน';color='house-red';} state.factorRisk={person,scores,total};save();const result=$('#factorResult');result.className=`home-result ${color}`;result.innerHTML=`<h3>${title}</h3><p>ผู้รับการประเมิน: <b>${safe(person.name)}</b> · อายุ ${safe(person.age)} ปี</p><p>คะแนนรวม <b>${total} / 33 คะแนน</b></p><p>${detail}</p>`;result.hidden=false;result.scrollIntoView({behavior:'smooth',block:'center'}); });
  const sarcItems=[
    ['การยกและเคลื่อนย้ายสิ่งของน้ำหนัก 4.5 กิโลกรัม',['ไม่ยาก','ยากเล็กน้อย','ยาก']],
  ...
  $('#startSarcF').addEventListener('click',()=>{renderSarcQuestions();$('#sarcForm').reset();renderIdentity('sarcIdentity',savedPerson('sarc'));$('#sarcResult').hidden=true;error('#sarcError','');showStep(9);});
  $('#sarcBackHome').addEventListener('click',()=>showStep(0));
  $('#startOsteoporosis').addEventListener('click',()=>{ $('#osteoporosisForm').reset(); renderIdentity('osteoporosisIdentity',savedPerson('osteoporosis')); $('#osteoporosisWeight').value=(state.osteoporosis||{}).weight??''; $('#osteoporosisResult').hidden=true; error('#osteoporosisError',''); showStep(12); });
  $('#osteoporosisBackHome').addEventListener('click',()=>showStep(0));
  $('#osteoporosisForm').addEventListener('submit',e=>{ e.preventDefault(); const person=readIdentity('osteoporosisIdentity'); const weightInput=$('#osteoporosisWeight').value; const weight=Number(weightInput); if(!person){error('#osteoporosisError','กรุณากรอกชื่อและอายุผู้รับการประเมิน');return;} if(weightInput===''||!Number.isFinite(weight)||weight<=0){error('#osteoporosisError','กรุณากรอกน้ำหนักตัวเป็นกิโลกรัม');return;} error('#osteoporosisError',''); const rawScore=.2*(weight-Number(person.age)); const score=Math.trunc(rawScore); let title,detail,color; if(score>-1){title='ความเสี่ยงโรคกระดูกพรุนระดับต่ำ';detail='Low risk · คะแนนมากกว่า -1';color='house-green';}else if(score>=-4){title='ความเสี่ยงโรคกระดูกพรุนระดับปานกลาง';detail='Moderate risk · คะแนนระหว่าง -4 ถึง -1';color='house-yellow';}else{title='ความเสี่ยงโรคกระดูกพรุนระดับสูง';detail='High risk · คะแนนน้อยกว่า -4';color='house-red';} state.osteoporosis={person,weight,rawScore,score,level:title};save(); const result=$('#osteoporosisResult'); result.className=`home-result ${color}`; result.innerHTML=`<h3>${title}</h3><p>ผู้รับการประเมิน: <b>${safe(person.name)}</b> · อายุ ${safe(person.age)} ปี · น้ำหนัก ${weight} กก.</p><p>0.2 × (${weight} − ${person.age}) = ${rawScore.toFixed(2)}</p><p>คะแนน OSTA หลังตัดเศษทศนิยม: <b>${score}</b></p><p>${detail}</p>`; result.hidden=false; appendGoogleSave(result,'osteoporosis'); result.scrollIntoView({behavior:'smooth',block:'center'}); });
  const stepTestNorms={male:[[65,87,115,'60–64 ปี'],[70,86,116,'65–69 ปี'],[75,80,110,'70–74 ปี'],[80,73,109,'75–79 ปี'],[85,71,103,'80–84 ปี'],[90,59,91,'85–89 ปี'],[Infinity,52,86,'90–94 ปี']],female:[[65,75,100,'60–64 ปี'],[70,73,107,'65–69 ปี'],[75,68,101,'70–74 ปี'],[80,68,100,'75–79 ปี'],[85,60,90,'80–84 ปี'],[90,55,85,'85–89 ปี'],[Infinity,44,72,'90–94 ปี']]};
  function stepTestNorm(age,gender){return stepTestNorms[gender].find(([limit])=>age<limit);}
  $('#startStepTest').addEventListener('click',()=>{ $('#stepTestForm').reset(); renderIdentity('stepTestIdentity',savedPerson('stepTest')); const old=(state.stepTest||{}).person||{}; setRadio('stepTestGender',old.gender||state.patient.gender||''); $('#stepTestReps').value=(state.stepTest||{}).reps??''; $('#stepTestResult').hidden=true; error('#stepTestError',''); showStep(13); });
  $('#stepTestBackHome').addEventListener('click',()=>showStep(0));
  $('#stepTestForm').addEventListener('submit',e=>{e.preventDefault();const identity=readIdentity('stepTestIdentity');const gender=radio('stepTestGender');const repsInput=$('#stepTestReps').value;const reps=Number(repsInput);if(!identity){error('#stepTestError','กรุณากรอกชื่อและอายุผู้รับการประเมิน');return;}if(!gender){error('#stepTestError','กรุณาเลือกเพศ');return;}if(repsInput===''||!Number.isFinite(reps)||reps<0){error('#stepTestError','กรุณากรอกจำนวนครั้งที่ทำได้');return;}error('#stepTestError','');const age=Number(identity.age),norm=stepTestNorm(age,gender),pass=reps>=norm[1],person={...identity,gender};const result=$('#stepTestResult');state.stepTest={person,reps,ageBand:norm[3],minimum:norm[1],referenceMaximum:norm[2],pass};save();result.className=`home-result ${pass?'house-green':'house-red'}`;result.innerHTML=`<h3>${pass?'ผ่านเกณฑ์':'ไม่ผ่านเกณฑ์'}</h3><p>ผู้รับการประเมิน: <b>${safe(person.name)}</b> · เพศ${sexText(gender)} · อายุ ${safe(person.age)} ปี</p><p>ทำได้ <b>${reps} ครั้ง</b> ภายใน 2 นาที</p><p>เกณฑ์อ้างอิง ${norm[3]} เพศ${sexText(gender)}: ${norm[1]}–${norm[2]} ครั้ง</p><p>${pass?'จำนวนครั้งไม่น้อยกว่าเกณฑ์ขั้นต่ำ':'จำนวนครั้งน้อยกว่าเกณฑ์ขั้นต่ำ'}</p>`;result.hidden=false;appendGoogleSave(result,'stepTest');result.scrollIntoView({behavior:'smooth',block:'center'});});
  const frailIllnessList=['โรคความดันโลหิตสูง','โรคเบาหวาน','โรคมะเร็ง (ไม่รวมมะเร็งผิวหนัง)','โรคปอดเรื้อรัง','โรคหัวใจขาดเลือด','ภาวะหัวใจล้มเหลว','โรคหลอดเลือดหัวใจตีบ','โรคหลอดเลือดสมอง','โรคข้ออักเสบ','โรคตับ','โรคไต'];
  function renderFrailtyIllnesses(){ $('#frailIllnesses').innerHTML=frailIllnessList.map((name,index)=>`<label class="risk-item frail-illness"><input type="checkbox" name="frailIllness" value="${index}"><span>${name}</span></label>`).join(''); }
  $('#startFrailty').addEventListener('click',()=>{ $('#frailtyForm').reset(); renderIdentity('frailtyIdentity',savedPerson('frailty')); renderFrailtyIllnesses(); const saved=state.frailty||{}; $('#frailCurrentWeight').value=saved.currentWeight??''; $('#frailPreviousWeight').value=saved.previousWeight??''; $('#frailtyResult').hidden=true; error('#frailtyError',''); showStep(14); });
  $('#frailtyBackHome').addEventListener('click',()=>showStep(0));
  $('#frailtyForm').addEventListener('submit',e=>{e.preventDefault();const person=readIdentity('frailtyIdentity');const fatigue=radio('frailFatigue'),stairs=radio('frailStairs'),walk=radio('frailWalk');const currentInput=$('#frailCurrentWeight').value,previousInput=$('#frailPreviousWeight').value,currentWeight=Number(currentInput),previousWeight=Number(previousInput);if(!person){error('#frailtyError','กรุณากรอกชื่อและอายุผู้รับการประเมิน');return;}if(fatigue===''||stairs===''||walk===''){error('#frailtyError','กรุณาเลือกคำตอบข้อ 1–3 ให้ครบ');return;}if(currentInput===''||previousInput===''||!Number.isFinite(currentWeight)||!Number.isFinite(previousWeight)||currentWeight<=0||previousWeight<=0){error('#frailtyError','กรุณากรอกน้ำหนักปัจจุบันและน้ำหนักเมื่อ 1 ปีก่อน');return;}error('#frailtyError','');const illnesses=$$('input[name="frailIllness"]:checked').map(input=>frailIllnessList[Number(input.value)]);const illnessScore=illnesses.length>=5?1:0;const weightLossPercent=Math.max(0,((previousWeight-currentWeight)/previousWeight)*100);const weightScore=weightLossPercent>=5?1:0;const total=Number(fatigue)+Number(stairs)+Number(walk)+illnessScore+weightScore;const frail=total>=3;state.frailty={person,fatigue:Number(fatigue),stairs:Number(stairs),walk:Number(walk),illnesses,illnessCount:illnesses.length,illnessScore,currentWeight,previousWeight,weightLossPercent,weightScore,total,frail};save();const result=$('#frailtyResult');result.className=`home-result ${frail?'house-red':'house-green'}`;result.innerHTML=`<h3>${frail?'เข้าเกณฑ์ภาวะเปราะบาง':'ยังไม่เข้าเกณฑ์ภาวะเปราะบาง'}</h3><p>ผู้รับการประเมิน: <b>${safe(person.name)}</b> · อายุ ${safe(person.age)} ปี</p><p>คะแนนรวม T-FRAIL <b>${total} / 5 คะแนน</b></p><p>โรคร่วม ${illnesses.length} โรค (${illnessScore} คะแนน) · น้ำหนักลด ${weightLossPercent.toFixed(1)}% (${weightScore} คะแนน)</p><p>${frail?'คะแนนตั้งแต่ 3 คะแนนขึ้นไป: มีภาวะเปราะบาง':'คะแนนน้อยกว่า 3 คะแนน: ยังไม่เข้าเกณฑ์ภาวะเปราะบาง'}</p>`;result.hidden=false;appendGoogleSave(result,'frailty');result.scrollIntoView({behavior:'smooth',block:'center'});});
  $('#sarcForm').addEventListener('submit',e=>{e.preventDefault();const person=readIdentity('sarcIdentity');const scores=sarcItems.map((_,index)=>radio(`sarc${index}`));const gender=radio('sarcGender');const calf=Number($('#calfCircumference').value);if(!person){error('#sarcError','กรุณากรอกชื่อและอายุผู้รับการประเมิน');return;}if(scores.some(x=>x==='')||!gender||$('#calfCircumference').value===''){error('#sarcError','กรุณาเลือกคำตอบให้ครบทั้ง 5 ข้อ ระบุเพศ และเส้นรอบวงน่อง');return;}error('#sarcError','');const sarcTotal=scores.reduce((sum,x)=>sum+Number(x),0);const cutoff=gender==='male'?34:33;const calfPass=calf>=cutoff;const sarcCalfTotal=sarcTotal+(calfPass?0:10);const lowRisk=sarcTotal<4&&calfPass;const result=$('#sarcResult');const color=lowRisk?'house-green':'house-red';const title=lowRisk?'ไม่พบความเสี่ยงต่อภาวะมวลกล้ามเนื้อน้อย':'ต้องสงสัยภาวะมวลกล้ามเนื้อน้อย';const advice=lowRisk?'แนะนำประเมินซ้ำใน 3 เดือน':'ควรได้รับการประเมินยืนยันและวางแผนดูแลโดยบุคลากรสุขภาพ';state.sarc={person,scores,sarcTotal,gender,calf,calfPass,sarcCalfTotal};save();result.className=`home-result ${color}`;result.innerHTML=`<h3>${title}</h3><p>ผู้รับการประเมิน: <b>${safe(person.name)}</b> · อายุ ${safe(person.age)} ปี</p><p>SARC-F <b>${sarcTotal} / 10 คะแนน</b> · เส้นรอบวงน่อง ${calf.toFixed(1)} ซม. (${calfPass?'ผ่านเกณฑ์':'ไม่ผ่านเกณฑ์'})</p><p>SARC-CalF รวม <b>${sarcCalfTotal} / 20 คะแนน</b></p><p>${advice}</p>`;result.hidden=false;result.scrollIntoView({behavior:'smooth',block:'center'});});
  const miniSections=[
  ...
  function updateOneLegScores(){
    const box=$('#oneLegScoreResult');
    if(!box)return;
    const side=(firstId,secondId,label)=>{
      const first=$('#'+firstId).value,second=$('#'+secondId).value;
      if(first===''||second==='')return null;
      const time=Math.min(Number(first),Number(second));
      return {label,time,score:oneLegScore(time)};
    };
    const left=side('leftTrial1','leftTrial2','ด้านซ้าย');
    const right=side('rightTrial1','rightTrial2','ด้านขวา');
    if(!left||!right){box.className='status neutral';box.textContent='กรอกเวลา Trial 1 และ Trial 2 ของทั้งซ้ายและขวา เพื่อให้ระบบคำนวณคะแนนอัตโนมัติ';return;}
    const score=Math.min(left.score,right.score);
    box.className=score===2?'status pass':score===1?'status neutral':'status fail';
    box.innerHTML=`${left.label}: ใช้เวลา ${left.time.toFixed(1)} วินาที = ${left.score} คะแนน<br>${right.label}: ใช้เวลา ${right.time.toFixed(1)} วินาที = ${right.score} คะแนน<br><b>คะแนนข้อ 3 = ${score} คะแนน</b> (ใช้คะแนนที่น้อยกว่าระหว่างซ้ายและขวา)`;
  }
  function calculatedMiniScore(index){
    if(index===2){
      ...
      const left=Math.min(Number(values[0]),Number(values[1]));
      const right=Math.min(Number(values[2]),Number(values[3]));
  ...
  function miniSpecialContent(index){
    if(index===2) return `<div class="mini-special"><p><b>Left:</b> Time in Seconds Trial 1 และ Trial 2 — ใช้เวลาที่น้อยกว่าเพื่อให้คะแนนด้านซ้าย</p><div class="field-grid"><label>Left Trial 1 (seconds)<input id="leftTrial1" type="number" min="0" step="0.1" inputmode="decimal"></label><label>Left Trial 2 (seconds)<input id="leftTrial2" type="number" min="0" step="0.1" inputmode="decimal"></label></div><p><b>Right:</b> Time in Seconds Trial 1 และ Trial 2 — ใช้เวลาที่น้อยกว่าเพื่อให้คะแนนด้านขวา</p><div class="field-grid"><label>Right Trial 1 (seconds)<input id="rightTrial1" type="number" min="0" step="0.1" inputmode="decimal"></label><label>Right Trial 2 (seconds)<input id="rightTrial2" type="number" min="0" step="0.1" inputmode="decimal"></label></div><p class="hint"><b>เกณฑ์คำนวณอัตโนมัติ</b> — ไม่ต้องกดเลือกคะแนน</p><div class="factor-options" style="margin-top:9px"><div class="factor-option"><span class="factor-score">2</span><span>Normal: 20 s.</span></div><div class="factor-option"><span class="factor-score">1</span><span>Moderate: &lt; 20 s.</span></div><div class="factor-option"><span class="factor-score">0</span><span>Severe: Unable.</span></div></div><p id="oneLegScoreResult" class="status neutral">กรอกเวลา Trial 1 และ Trial 2 ของทั้งซ้ายและขวา เพื่อให้ระบบคำนวณคะแนนอัตโนมัติ</p><p class="hint">ระบบเลือกเวลาที่น้อยกว่าจาก Trial 1 และ Trial 2 ของแต่ละข้างก่อน แล้วใช้คะแนนที่ต่ำกว่าระหว่างซ้ายและขวาเป็นคะแนนข้อ 3</p></div>`;
  ...
  function renderMiniBest(){let counter=0;$('#miniBestQuestions').innerHTML=miniSections.map(([title,items])=>`<article class="factor-card"><h3>${title} · คะแนนเต็ม ${items.length*2}</h3><div class="factor-options">${items.map(([en,th])=>{const i=counter++;return `<div class="factor-option"><span class="factor-score">${i+1}</span><span><b>${en}</b><br>${th}${miniSpecialContent(i)}</span></div>`;}).join('')}</div></article>`).join('');$$('#miniTugTime,#miniDualTugTime').forEach(input=>input.addEventListener('input',updateDualTaskDifference));$$('#leftTrial1,#leftTrial2,#rightTrial1,#rightTrial2').forEach(input=>input.addEventListener('input',updateOneLegScores));updateOneLegScores();}
  $('#startMiniBest').addEventListener('click',()=>{renderMiniBest();$('#miniBestForm').reset();renderIdentity('miniBestIdentity',savedPerson('miniBest'),false);$('#miniBestAge').value=(state.miniBest||{}).age??state.patient.age??'';$('#miniBestResult').hidden=true;error('#miniBestError','');showStep(10);});
  $('#miniBestBackHome').addEventListener('click',()=>showStep(0));
  $('#miniBestForm').addEventListener('submit',e=>{e.preventDefault();const person=readIdentity('miniBestIdentity',false);const ageInput=$('#miniBestAge').value;const age=Number(ageInput);const scores=Array.from({length:14},(_,i)=>calculatedMiniScore(i));if(!person){error('#miniBestError','กรุณากรอกชื่อผู้รับการประเมิน');return;}if(ageInput===''||!Number.isFinite(age)||age<0){error('#miniBestError','กรุณาระบุอายุผู้รับการประเมิน');return;}if(scores.some(x=>x==='')){error('#miniBestError','กรุณาให้คะแนนและกรอกข้อมูลครบทั้ง 14 รายการ');return;}error('#miniBestError','');const values=scores.map(Number);const sectionScores=[values.slice(0,3).reduce((a,b)=>a+b,0),values.slice(3,6).reduce((a,b)=>a+b,0),values.slice(6,9).reduce((a,b)=>a+b,0),values.slice(9,14).reduce((a,b)=>a+b,0)];const total=sectionScores.reduce((a,b)=>a+b,0);const cutoff=age>=60?18:22;const atRisk=total<cutoff;const ageCriterion=age>=60?'อายุ 60 ปีขึ้นไป: คะแนนน้อยกว่า 18 คะแนน มีความเสี่ยงล้ม':'อายุต่ำกว่า 60 ปี: ใช้เกณฑ์คะแนนน้อยกว่า 22 คะแนน มีความเสี่ยงล้ม';state.miniBest={...(state.miniBest||{}),person:{name:person.name,age},age};save();const result=$('#miniBestResult');result.innerHTML=miniSections.map(([title,items],i)=>`<article class="mini-result-card"><div class="mini-result-picture"></div><div class="mini-result-copy"><h3>${title}</h3><p><b>${sectionScores[i]} / ${items.length*2} คะแนน</b></p></div></article>`).join('')+`<div class="home-result ${atRisk?'house-red':'house-green'}" style="grid-column:1/-1"><h3>คะแนนรวม Mini-BESTest</h3><p>ผู้รับการประเมิน: <b>${safe(person.name)}</b> · อายุ ${age} ปี</p><p><b>${total} / 28 คะแนน</b></p><p><b>${atRisk?'มีความเสี่ยงล้ม':'ไม่เข้าเกณฑ์เสี่ยงล้มตามคะแนน Mini-BESTest'}</b></p><p>${ageCriterion}</p></div>`;result.hidden=false;result.scrollIntoView({behavior:'smooth',block:'center'});});
  ...
  $('#homeSafetyForm').addEventListener('submit',()=>{
    ...
    state.homeSafety={...(state.homeSafety||{}),riskCount:risks.length,risks};save();
  ...
  $('#miniBestForm').addEventListener('submit',()=>{
    ...
    state.miniBest={...(state.miniBest||{}),age,scores:values,sectionScores,total,cutoff,atRisk:total<cutoff};save();
