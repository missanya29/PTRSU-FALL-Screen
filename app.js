(() => {
  ...
  $('#startFeedback').addEventListener('click',()=>{ $('#feedbackForm').reset(); $('#feedbackResult').hidden=true; error('#feedbackError',''); showStep(11); });
  $('#feedbackBackHome').addEventListener('click',()=>showStep(0));
  $('#feedbackForm').addEventListener('submit',e=>{e.preventDefault();const name=$('#feedbackName').value.trim(),workplace=$('#feedbackWorkplace').value.trim(),message=$('#feedbackMessage').value.trim();if(!name||!workplace||!message){error('#feedbackError','กรุณากรอกชื่อ สถานที่ทำงาน และข้อเสนอแนะให้ครบ');return;}state.feedback={name,workplace,message,savedAt:new Date().toISOString()};save();error('#feedbackError','');const subject=encodeURIComponent('Feedback: PTRSU FALL Screen');const body=encodeURIComponent(`ชื่อ: ${name}\nสถานที่ทำงาน: ${workplace}\n\nFeedback / ข้อเสนอแนะ:\n${message}`);const result=$('#feedbackResult');result.innerHTML=`<h3>กำลังเปิดอีเมล</h3><p>โปรดตรวจทานข้อความ แล้วกดส่งถึงผู้พัฒนา</p>`;result.hidden=false;window.location.href=`mailto:missanya2929@gmail.com?subject=${subject}&body=${body}`;});
