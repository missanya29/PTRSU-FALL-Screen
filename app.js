(() => {
  ...
  function showStep(step){
    state.step = Math.max(0, Math.min(10, step)); save();
  ...
  function calculatedMiniScore(index){
    ...
    if(index===13){
      const tug=Number($('#miniTugTime')?.value),dual=Number($('#miniDualTugTime')?.value);
      if(!Number.isFinite(tug)||!Number.isFinite(dual)||tug<=0||dual<0) return '';
      if($('#miniDualStopped')?.checked) return '0';
      return String(((dual-tug)/tug*100)>10?1:2);
    }
  ...
  function miniSpecialContent(index){
    ...
    if(index===13) return `<div class="mini-special"><div class="field-grid"><label>Timed Up & Go (seconds)<input id="miniTugTime" type="number" min="0" step="0.1" inputmode="decimal"></label><label>Timed Up & Go with Dual Task (seconds)<input id="miniDualTugTime" type="number" min="0" step="0.1" inputmode="decimal"></label></div><label class="choice"><input id="miniDualStopped" type="checkbox"> Stops counting while walking OR stops walking while counting</label><p class="hint">ระบบคิด 2 คะแนนเมื่อเวลา Dual Task เพิ่มขึ้นไม่เกิน 10%; 1 คะแนนเมื่อเพิ่มขึ้นมากกว่า 10%; หรือ 0 คะแนนเมื่อหยุดนับ/หยุดเดิน</p></div>`;
  ...
  function renderMiniBest(){let counter=0;$('#miniBestQuestions').innerHTML=miniSections.map(([title,items])=>`<article class="factor-card"><h3>${title} · คะแนนเต็ม ${items.length*2}</h3><div class="factor-options">${items.map(([en,th])=>{const i=counter++;return `<div class="factor-option"><span class="factor-score">${i+1}</span><span><b>${en}</b><br>${th}${miniSpecialContent(i)}</span></div>`;}).join('')}</div></article>`).join('');}
  $('#startMiniBest').addEventListener('click',()=>{renderMiniBest();$('#miniBestResult').hidden=true;error('#miniBestError','');showStep(10);});
  $('#miniBestBackHome').addEventListener('click',()=>showStep(0));
  $('#miniBestForm').addEventListener('submit',e=>{e.preventDefault();const scores=Array.from({length:14},(_,i)=>calculatedMiniScore(i));if(scores.some(x=>x==='')){error('#miniBestError','กรุณาให้คะแนนและกรอกข้อมูลครบทั้ง 14 รายการ');return;}error('#miniBestError','');const values=scores.map(Number);const sectionScores=[values.slice(0,3).reduce((a,b)=>a+b,0),values.slice(3,6).reduce((a,b)=>a+b,0),values.slice(6,9).reduce((a,b)=>a+b,0),values.slice(9,14).reduce((a,b)=>a+b,0)];const result=$('#miniBestResult');result.innerHTML=miniSections.map(([title,items],i)=>`<article class="mini-result-card"><div class="mini-result-picture"></div><div class="mini-result-copy"><h3>${title}</h3><p><b>${sectionScores[i]} / ${items.length*2} คะแนน</b></p></div></article>`).join('')+`<div class="home-result house-green" style="grid-column:1/-1"><h3>คะแนนรวม Mini-BESTest</h3><p><b>${sectionScores.reduce((a,b)=>a+b,0)} / 28 คะแนน</b></p></div>`;result.hidden=false;result.scrollIntoView({behavior:'smooth',block:'center'});});
