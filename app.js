(() => {
  ...
  function renderMiniBest(){let counter=0;$('#miniBestQuestions').innerHTML=miniSections.map(([title,items])=>`<article class="factor-card"><h3>${title} · คะแนนเต็ม ${items.length*2}</h3><div class="factor-options">${items.map(([en,th])=>{const i=counter++;return `<div class="factor-option"><span class="factor-score">${i+1}</span><span><b>${en}</b><br>${th}<div class="factor-options" style="margin-top:9px">${miniCriteria[i].map(([score,text])=>`<label class="factor-option"><input type="radio" name="mini${i}" value="${score}"><span class="factor-score">${score}</span><span>${text}</span></label>`).join('')}</div></span></div>`;}).join('')}</div></article>`).join('');}
